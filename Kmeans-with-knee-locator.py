...
df=pd.read_csv('seeds_data.csv', header = None)
...
kmeans3 = KMeans(init="k-means++", n_clusters=3, n_init=10, random_state=42)
y_kmeans3 = kmeans3.fit(scaled_data)


y_pred = np.array(kmeans3.labels_)
y_true = np.array(df.iloc[:, [7]])
...
kl = KneeLocator(range(1, 8), sse, curve="convex", direction="decreasing")
...
#plt.scatter(kmeans3.cluster_centers_[:,0] ,kmeans3.cluster_centers_[:,1], kmeans3.cluster_centers_[:,2], kmeans3.cluster_centers_[:,3], kmeans3.cluster_centers_[:,4], color='black')